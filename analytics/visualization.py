...
def plot_utilization(nodes: List[StorageNode], save_path: Optional[str] = "utilization.png") -> None:
    ...
    for i, (b, v) in enumerate(zip(bars, vals)):
        ...
        if cap is None:
            ...
            pct = ratios[i] * 100
            label = f"{v}/{cap} ({pct:.0f}%)"
        plt.text(b.get_x() + b.get_width() / 2, v + 0.1, label, ha="center", va="bottom")
    plt.tight_layout()
    if save_path:
